...
import { buildSmirkOutcomeContract, SMIRK_RAILWAY_CALLBACK_KEY, SMIRK_RAILWAY_WORKSPACE_ID } from "../shared/smirkIntegrationContract";

describe("SMIRK integration contract", () => {
  it("uses the verified Railway callback variable and workspace binding", () => {
    expect(SMIRK_RAILWAY_CALLBACK_KEY).toBe("VELVET_ALCHEMY_HANDOFF_API_KEY");
  ...
  it("publishes the exact outcome endpoint and authorization placeholder", () => {
    ...
    expect(contract).toContain("Authorization: Bearer <VELVET_ALCHEMY_HANDOFF_API_KEY>");
